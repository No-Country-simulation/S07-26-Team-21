...
class UserEvaluation(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("p1_visibilidad_herramientas  BETWEEN 1 AND 5", name="ck_p1_range"), #Usamos Between en vez de Field(ge=1, le=5) por que Field es para Pydantic y no para SQLAlchemy.
        CheckConstraint("p2_visibilidad_dashboards BETWEEN 1 AND 5", name="ck_p2_range"),
        CheckConstraint("p3_visibilidad_telemetry BETWEEN 1 AND 5", name="ck_p3_range"),
        CheckConstraint("p4_friccion_energia BETWEEN 1 AND 5", name="ck_p4_range"),
        CheckConstraint("p5_friccion_cooling BETWEEN 1 AND 5", name="ck_p5_range"),
        CheckConstraint("p6_latencia_manual BETWEEN 1 AND 5", name="ck_p6_range"),
        CheckConstraint("p7_latencia_semi_auto BETWEEN 1 AND 5", name="ck_p7_range"),
        CheckConstraint("p8_latencia_full_auto BETWEEN 1 AND 5", name="ck_p8_range"),
        CheckConstraint("p9_auto_cuant_pue BETWEEN 1 AND 5", name="ck_p9_range"),
        CheckConstraint("p10_auto_cuant_utilizacion BETWEEN 1 AND 5", name="ck_p10_range"),
        CheckConstraint("p11_bloqueantes_staffing BETWEEN 1 AND 5", name="ck_p11_range"),
        CheckConstraint("p12_bloqueantes_supply BETWEEN 1 AND 5", name="ck_p12_range"),
        CheckConstraint("p13_bloqueantes_energy BETWEEN 1 AND 5", name="ck_p13_range"),
        CheckConstraint("p14_bloqueantes_regulacion BETWEEN 1 AND 5", name="ck_p14_range"),
        CheckConstraint("p15_bloqueantes_expertise BETWEEN 1 AND 5", name="ck_p15_range"),
    )
    ...
    # CONTEXTO
    facility_size: Mapped[str] = mapped_column(String(20), nullable=False, index=True)  #El modelo industry_benchmark.py usa indices para realizar consultas rápidas por eso aquí también los usamos.
    ...
    p2_visibilidad_dashboards: Mapped[int] = mapped_column(Integer, nullable=False)
    p3_visibilidad_telemetry: Mapped[int] = mapped_column(Integer, nullable=False)
    ...
    p10_auto_cuant_utilizacion: Mapped[int] = mapped_column(Integer, nullable=False)
 
    p11_bloqueantes_staffing: Mapped[int] = mapped_column(Integer, nullable=False)
    ...
    p14_bloqueantes_regulacion: Mapped[int] = mapped_column(Integer, nullable=False)
    p15_bloqueantes_expertise: Mapped[int] = mapped_column(Integer, nullable=False)

    # Sub-scores Likert Promediados (Cacheados)
    score_visibilidad: Mapped[float] = mapped_column(Float, nullable=True) #esto sirve para guardar el promedio de las preguntas p1, p2 y p3 en la base de datos, evitando tener que calcularlo cada vez que se necesite.
    ...
    def __repr__(self) -> str:    #funcion que devuelve una representación del objeto UserEvaluation, mostrando id, tamaño de instalación, región y fecha de creación. Esto es útil para depuración y registro.
        return (
